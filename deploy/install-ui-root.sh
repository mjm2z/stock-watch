...
if [[ ${1:-} != --run ]]; then
  systemd-run --no-block --collect --unit=stock-watch-ui-install \
    --property=Type=oneshot --property=TimeoutStartSec=5min \
    /bin/bash "${SOURCE}/deploy/install-ui-root.sh" --run
  echo 'UI update started independently of SSH.'
  echo 'Progress: journalctl -u stock-watch-ui-install --no-pager -n 30'
  exit 0
fi
...
readonly saved="/var/backups/home-ops-code/stock-watch-ui-$(date -u +%Y%m%dT%H%M%SZ)"
...
for endpoint in api/health api/dashboard/execution api/dashboard/liquidity api/dashboard/portfolio api/dashboard/signals signals operations portfolio research backtests; do
...
PY
echo "UI installed and verified. Rollback files: ${saved}"
